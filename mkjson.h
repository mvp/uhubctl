...
// Null-terminated array version - no need to count parameters
typedef struct {
    enum mkjson_value_type type;
    const char *key;  // For objects only, can be NULL for arrays
    union {
        const char *str_val;
        char *str_free_val;
        int int_val;
...
extern char *mkjson_array( enum mkjson_container_type otype, const mkjson_arg *args );
extern char *mkjson_array_pretty( enum mkjson_container_type otype, const mkjson_arg *args, int indent_size );
